...
5. **라우트 전환 표에는 간선만 적는다** — 전체 여정 서술은
   `../../planning/user-flows.md`의 일이다.
...
6. **이 문서가 프론트엔드 체인의 마지막이다.** 다음 링크는
   `../../planning/traceability.md`를 가리킨다.

> **단계 5 완료**: 상태 흐름을 작성했습니다. 검토 후 유저 플로우 갱신으로
> 진행할까요?

### 단계 6: 기획 유저 플로우의 서버 흐름 갱신
...
2. 정상 경로 표의 `서버 흐름` 열에서 **`설계 대기`를 실제 `FLOW-` 키 링크로
   바꾼다.** `—`로 적힌 칸은 건드리지 않는다.
3. **그 외의 내용은 바이트 단위로 보존한다.** 플로우를 다시 그리거나 경로를
   고치지 않는다 — 사람이 결정한 기획이다.
4. 설계상 필요한데 기획 플로우에 없는 화면 전환을 발견하면 **문서를 고치지
   말고** 완료 리포트에 "기획 플로우와 설계가 어긋나는 지점"으로 보고한다.
5. `## 문서 정보`의 `최종 수정일`과 버전 이력을 갱신한다 (`common-rules.md`
   §6.4).
...
3. E2E는 기획의 유저 플로우(`UF-`) 하나에 하나가 기본이다. **모든 인수 기준을 E2E로
...
`common-rules.md` §4에 다음을 더한다: 부여한 `FLOW-` 키 목록, 라우트를
받지 못한 `SCR-` 화면 목록, 기획 플로우와 설계가 어긋나는 지점.
