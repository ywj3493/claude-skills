<!--
템플릿: 프론트엔드 설계 문서 (user-flows.md)

내비게이션 규칙은 `${CLAUDE_PLUGIN_ROOT}/references/document-order.md` 참조.
프론트엔드 체인의 마지막 문서이므로 다음 링크는
`../../planning/traceability.md`다.

**책임 경계 — 이 문서는 사용자가 보는 것만 다룬다.**
화면 전환과 사용자 행동, 그리고 사용자에게 보이는 결과.

**백엔드 호출을 여기 그리지 않는다.** 서버 처리가 필요한 지점은 해당
`FLOW-` 키를 링크만 한다. 이 규칙을 어기면 프론트엔드 유저 플로우와 백엔드
시퀀스 다이어그램이 같은 내용을 두 번 그리게 되고, 코드가 바뀔 때 두 곳이
어긋난다.

**프레임워크 중립 어휘**를 쓴다.
-->

> [← 상태 흐름](state-flow.md) | [다음: 추적성 →](../../planning/traceability.md)

# 유저 플로우

> **도메인**: <!-- 도메인 이름 -->
>
> **계층**: 프론트엔드

---

## 목차

1. [개요](#개요)
2. [플로우 목록](#플로우-목록)
3. [플로우 상세](#플로우-상세)
4. [진입점](#진입점)

---

## 개요

<!-- 이 도메인에서 사용자가 하는 일을 2~4문장으로. -->

---

## 플로우 목록

| 플로우 | 관련 스토리 | 시작 라우트 | 종료 라우트 | 주 액터 |
|---|---|---|---|---|
| | US-NN | | | |

---

## 플로우 상세

<!--
플로우 하나당 한 섹션. 유저 스토리 하나에 대응하는 것이 보통이다.

`flowchart TD`로 그린다. 노드는 **사용자가 보는 화면이나 결정**이고,
간선은 **사용자 행동**이다. 서버 처리는 노드 하나로 뭉뚱그리고 그 안을
그리지 않는다.
-->

### <플로우 이름>

| 항목 | 내용 |
|---|---|
| **관련 스토리** | US-NN |
| **주 액터** | |
| **사전 조건** | |
| **성공 기준** | |

```mermaid
flowchart TD
    START([주문 목록 화면]) --> PICK{주문 선택}
    PICK -->|주문 클릭| DETAIL[주문 상세 화면]
    DETAIL --> CANCEL{취소 가능?}
    CANCEL -->|가능| CONFIRM[취소 확인 대화상자]
    CANCEL -->|불가| DISABLED[취소 버튼 비활성 안내]
    CONFIRM -->|확인| PROCESSING[처리 중 표시]
    CONFIRM -->|취소| DETAIL
    PROCESSING --> RESULT{처리 결과}
    RESULT -->|성공| DONE([취소 완료 안내])
    RESULT -->|실패| ERROR[오류 안내와 재시도]
    ERROR --> CONFIRM
```

**정상 경로**

| 단계 | 사용자 행동 | 보이는 결과 | 관련 라우트 | 서버 흐름 |
|---|---|---|---|---|
| 1 | | | [라우팅](routing.md#라우트-목록) | <!-- FLOW-<도메인>-<오퍼레이션> 링크. 서버 호출이 없으면 — --> |

**대안 경로**

<!-- 목표에는 도달하지만 경로가 다른 경우. 없으면 소제목까지 삭제한다. -->

| 분기 지점 | 조건 | 이후 경로 |
|---|---|---|
| | | |

**예외 경로**

<!--
목표에 도달하지 못하는 경우와 그때 사용자가 보는 것.
오류 코드는 api-interface.md의 오류 모델을 링크하고 여기서 정의하지 않는다.
-->

| 발생 지점 | 원인 | 사용자에게 보이는 것 | 복구 수단 |
|---|---|---|---|
| | | | |

---

## 진입점

<!--
사용자가 이 도메인의 플로우에 들어오는 경로. 다른 도메인에서 넘어오는
경우가 있으면 domain-map.md의 횡단 흐름과 일치해야 한다.
-->

| 진입 경로 | 도착 플로우 | 전달 맥락 | 근거 |
|---|---|---|---|
| <!-- 내비게이션 메뉴 / 외부 링크 / 다른 도메인 --> | | | |

---

## 읽은 소스

<!-- 역방향 스킬 전용. 순방향 생성 시 제목까지 통째로 삭제한다. -->

---

## 관련 문서

- [라우팅](routing.md) — 플로우가 지나가는 라우트
- [렌더링 흐름](render-flow.md) — 각 화면이 그려지는 과정
- [상태 흐름](state-flow.md) — 플로우 중 유지되는 상태

### 참고 문서

- [유저 스토리](../../planning/user-stories.md) — 이 플로우가 실현하는 스토리
- [인터페이스 계약](../../planning/api-interface.md) — 오류 모델
- [도메인 지도](../../../domain-map.md) — 여러 도메인을 지나는 여정

---

## 문서 정보

| 항목 | 값 |
|---|---|
| **작성일** | YYYY-MM-DD |
| **최종 수정일** | YYYY-MM-DD |
| **상태** | 초안 |
| **분석 기준 커밋** | |
| **참고 문서** | |

**버전 이력**:

- 1.0.0 (YYYY-MM-DD): 최초 작성

---
> **전체 문서**
> [요구사항](../../planning/requirements.md) |
> [유저 스토리](../../planning/user-stories.md) |
> [인터페이스 계약](../../planning/api-interface.md) |
> [FSD 구조](fsd-structure.md) |
> [라우팅](routing.md) |
> [UI 구성](component-tree.md) |
> [렌더링 흐름](render-flow.md) |
> [상태 흐름](state-flow.md) |
> **유저 플로우** |
> [추적성](../../planning/traceability.md) |
> [테스트 명세](../../verification/test-spec.md)
