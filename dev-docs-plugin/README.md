...
from code that does. A third, lighter skill, `flow-diagram`, skips the full
...
| skill | `/dev-docs:flow-diagram` | Single-artifact skill for after implementation: writes or merges ONE Source-Linked sequence diagram for the flow just built or changed into the domain's `design/sequence-diagram.md`. No tier system, no review gates, no `doc-verifier` loop — the lightweight complement to the two full pipelines |
...
/dev-docs:init-docs              docs/ structure + source language + .claude/rules + CLAUDE.md
  ...
  └─ /dev-docs:flow-diagram          one Source-Linked sequence diagram for a change just made
...
`${CLAUDE_PLUGIN_ROOT}/templates/` (`flow-diagram` reuses
...
`skills/init-docs/scripts/`, and `domain-overview` bundles its single
template under `skills/domain-overview/references/` — both follow the
skill-local pattern because no other skill consumes them):
